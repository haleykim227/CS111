...
void input_handling(char* mini_buffer) {
	...
	if(strcmp(mini_buffer, "OFF") == 0) {
		...
		int SSL_write_ret = SSL_write(ssl, shutdown_report, strlen(shutdown_report));
		if (SSL_write_ret < 0) {
			fprintf(stderr, "Error: SSL_write() on shutdown report\n");
		}
		if (log_flag) {
			dprintf(4, "OFF\n");
...
int main (int argc, char *argv[]) {
    ...
    if (!SSL_set_fd(ssl, sockfd)) {
        fprintf(stderr, "Error: unable to associate fd w/SSL\n");
        exit(2);
    }
    if (SSL_connect(ssl) != 1) {
  	...
  	int SSL_write_ret1 = SSL_write(ssl, id_buffer, strlen(id_buffer));
  	if (SSL_write_ret1 < 0) {
  		fprintf(stderr, "Error: SSL_write() on ID number\n");
  	}
  	dprintf(logfd, "ID=%s\n", id_arg);
	...
	char first_report[256];
	float first_temp = get_temp();
	struct tm* log_time;
	log_time = localtime(&(first_time.tv_sec));
	sprintf(first_report, "%02d:%02d:%02d %.1f\n", log_time->tm_hour, 
		log_time->tm_min, log_time->tm_sec, first_temp);
	int SSL_write_ret2 = SSL_write(ssl, first_report, strlen(first_report));
	if (SSL_write_ret2 < 0) {
		fprintf(stderr, "Error: SSL_write() on first report\n");
	}
	if (log_flag) {
	...
	while(1) {
		...
		if (difftime_ret >= period_arg) {
			...
			if (report_flag) {
				sprintf(reports, "%02d:%02d:%02d %.1f\n", log_time->tm_hour, 
					log_time->tm_min, log_time->tm_sec, first_temp);
				int SSL_write_ret3 = SSL_write(ssl, reports, strlen(reports));
				if (SSL_write_ret3 < 0) {
			...
			first_time.tv_sec = second_time.tv_sec;
		 	...
		 	// resetting reports array to fill in again
		 	memset(reports, 0, 256);
		}
		...
		if(input[0].revents & POLLIN) {
			ssize_t bytes_read = SSL_read(ssl, buffer, 128);
			if (bytes_read < 0) {
